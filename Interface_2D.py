...
import numpy as np
from main_2D import PressureMapThread2D
...
class TabContent2D(QWidget):
    ...
    def initUI(self, title):
        ...
        self.xv = np.linspace(0, self.X_dimension, self.resolution)
        self.yv = np.linspace(0, self.Y_dimension, self.resolution)
        self.Xm, self.Ym = np.meshgrid(self.xv, self.yv)

        self.X_Sensors = np.array(
            [100, 18, 47, 100, 133, 182, 55, 86, 114, 145, 34, 72, 100, 128, 166, 55, 86, 114, 145, 28, 67, 100, 133,
             182, 100])
        self.Y_Sensors = np.array(
            [2, 46, 42, 48, 42, 30, 75, 75, 75, 75, 100, 100, 100, 100, 100, 125, 125, 125, 125, 154, 158, 152, 158,
             170, 198])

        self.thread = PressureMapThread2D(self.port, self.baud_rate, self.X_Sensors, self.Y_Sensors, self.Xm, self.Ym)
    ...
    def start_recording(self):
        ...
        if isinstance(main_window, Main2D):
    ...
    def save_to_file(self, filePath):
        try:
            with open(filePath, 'w') as file:
                for elapsed_time, Xm, Ym, Zm in self.data_buffer:
                    for x_row, y_row, z_row in zip(Xm, Ym, Zm):
                        for x, y, z in zip(x_row, y_row, z_row):
                            file.write(f"{elapsed_time:.3f}: {x}, {y}, {z}\n")
            self.data_buffer = []
    ...
    def collect_data(self, Xm, Ym, Zm):
        elapsed_time = self.recording_timer.elapsed() / 1000.0
        self.data_buffer.append((elapsed_time, Xm.copy(), Ym.copy(), Zm.copy()))

    def plot(self, Xm, Ym, Zm):
        self.figure.clear()
        ax = self.figure.add_subplot(111)
        c = ax.contourf(Xm, Ym, Zm, cmap='viridis')
        ...
        self.figure.subplots_adjust(left=0.1, right=1.04, top=0.95, bottom=0.1)
        self.canvas.draw()

    def initialize_plot(self):
        default_Zm = np.zeros_like(self.Xm)
        self.plot(self.Xm, self.Ym, default_Zm)
        self.stop_button.button.setEnabled(False)
...
class Main2D(QMainWindow):
    ...
    def addTab(self):
        ...
        new_tab = TabContent2D(title=title)
    ...
    def removeTab(self, index):
        ...
        if tab.recording_active:
            QMessageBox.critical(self, "Error",
                                 "Recording is active. Please stop the recording before deleting this tab.")
            return
        ...
        for tab_key, tab_data in data.items():
            if tab_key.startswith('Tab_'):
                if data.get('plot_type') == '2D':
                    ...
                    from Interface_3D import TabContent3D
